...
const WalkRoom = () => {
  ...
  useEffect(() => {
    ...
    // Floor with grey color
    const floorGeometry = new THREE.PlaneGeometry(ROOM_SIZE.width, ROOM_SIZE.depth);
    const floorMaterial = new THREE.MeshPhongMaterial({ color: 0x808080 }); // Grey
    ...
    // Ceiling with grey color
    const ceiling = new THREE.Mesh(
      new THREE.PlaneGeometry(ROOM_SIZE.width, ROOM_SIZE.depth),
      new THREE.MeshPhongMaterial({ color: 0x808080 }) // Grey
    );
    ...
    const animate = () => {
      ...
      if (move.current.forward) {
        direction.z += 1; // Forward movement (positive Z)
        moved = true;
      }
      if (move.current.backward) {
        direction.z -= 1; // Backward movement (negative Z)
        moved = true;
      }
    ...
    const handleKeyDown = (e) => {
      switch (e.code) {
        case "KeyW":
        case "ArrowUp":
          move.current.forward = true; // W moves forward
          break;
        case "KeyS":
        case "ArrowDown":
          move.current.backward = true; // S moves backward
          break;
        case "KeyA":
        case "ArrowLeft":
